...
class ReturningsController < ApplicationController
  ...
  def create
    if ReturningService.new(book: @book, user: current_user).call
      respond_to do |format|
        format.html { redirect_to(@book, notice: "Book was successfully returned.") }
        format.json { render(json: @book, status: :created) }
        format.turbo_stream do
          render(turbo_stream: turbo_stream.replace("book", partial: "books/returned", locals: { book: @book }))
        end
      end
    else
      respond_to do |format|
